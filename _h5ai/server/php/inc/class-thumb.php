...
class Thumb {
	...
	// Make our terrible hack work, otherwise it things all previews have a height of 0
	if ($height == 0) {
	    $height = round($width / 1.334);
	}
	...
	// Make the file path hash relative to the web root, so that we can move
	// the site around without having to regen thumbnails
	$rel_path = substr($source_path, strlen($_SERVER['DOCUMENT_ROOT']));
        $name = "thumb-" . sha1($rel_path) . "-" . $width . "x" . $height . ".jpg";
        $thumb_path = $this->thumbs_path . "/" . $name;
        $thumb_url = $this->thumbs_href . "/" . $name;

	//error_log('sourcepath:'.$source_path);
	//error_log('relpath:'.$rel_path);
	//error_log('sha1:'.sha1($rel_path));
	//error_log('thumbpath:'.$thumb_path);
	//error_log('thumburl:'.$thumb_url);
	...
	// Make the file path hash relative to the web root, so that we can move
	// the site around without having to regen thumbnails
	$rel_path = substr($path, strlen($_SERVER['DOCUMENT_ROOT']));
        $capture_path = $this->thumbs_path . "/capture-" . sha1($rel_path) . ".jpg";
	//error_log($source_path);
	//error_log($rel_path);
	//error_log($capture_path);
